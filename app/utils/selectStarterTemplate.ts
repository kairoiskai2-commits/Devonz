...
function buildDirectoryHint(files: Array<{ path: string }>): string {
  ...
  const ignore = new Set(['.git/', '.github/', '.veyra/', 'node_modules/', '.vscode/']);
...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .veyra
  filteredFiles = filteredFiles.filter((x) => !x.path.startsWith('.veyra'));

  // check for ignore file in .veyra folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.veyra') && x.name === 'ignore');
...
Veyra is initializing your project with the required files using the ${displayName} template.
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.veyra')).find((x) => x.name === 'prompt');
