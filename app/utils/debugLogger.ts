...
/** Augment Window with veyra-specific globals */
declare global {
  interface Window {
    __veyra_workbench_store?: { get?: () => Record<string, unknown> };
  }
...
class DebugLogger {
  ...
  private async _collectAppInfo(): Promise<AppInfo> {
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        const workbenchStore = window.__veyra_workbench_store;
  ...
  private _getCurrentModel(): string {
    try {
      ...
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('veyra_current_model');
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('veyra_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('veyra_project_type');
  ...
  private _getGitInfoFallback(): AppInfo['gitInfo'] {
    try {
      ...
      const stored = localStorage.getItem('veyra_git_info');
