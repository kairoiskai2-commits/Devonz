...
import { getOpenAILikeKeyPool } from '~/lib/modules/llm/providers/openai-like';
import {
...
`;
  ...
  try {
    ...
    // ── Key-rotation retry for OpenAILike auth/rate-limit errors ──────────
    if (
      provider.name === 'OpenAILike' &&
      (errorCategory === 'auth_failure' || errorCategory === 'rate_limit')
    ) {
      const sEnv = serverEnv as Record<string, string | undefined> | undefined;
      const pool = getOpenAILikeKeyPool(apiKeys, sEnv ?? {});

      if (pool && pool.size > 1) {
        const allKeys = pool.getAll();
        let keyRotationSucceeded = false;

        for (const rotateKey of allKeys) {
          logger.info(`OpenAILike key rotation: trying key ...${rotateKey.slice(-6)}`);

          try {
            const rotatedInstance = (provider as any).getModelInstance({
              model: modelDetails.name,
              serverEnv,
              apiKeys,
              providerSettings,
              _overrideKey: rotateKey,
            });

            const rotatedResult = await _streamText({ ...streamParams, model: rotatedInstance });
            await probeStreamForErrors(rotatedResult);

            pool.reportSuccess(rotateKey);
            logger.info(`OpenAILike key rotation succeeded with key ...${rotateKey.slice(-6)}`);
            result = rotatedResult;
            keyRotationSucceeded = true;
            ...
            pool.reportError(rotateKey);
            logger.warn(
              `OpenAILike key ...${rotateKey.slice(-6)} failed: ` +
                `${rotateErr instanceof Error ? rotateErr.message : String(rotateErr)}`,
            );
          }
        }

        if (!keyRotationSucceeded) {
          logger.error('OpenAILike: all keys exhausted during rotation');
        }
      }
    }
