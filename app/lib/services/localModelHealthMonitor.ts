...
const logger = createScopedLogger('HealthMonitor');
...
export class LocalModelHealthMonitor extends SimpleEventEmitter {
  ...
  private async _checkOllamaHealth(baseUrl: string, signal: AbortSignal): Promise<HealthCheckResult> {
    try {
      logger.debug(`Checking Ollama at ${baseUrl}`);
      ...
      logger.info(`Ollama healthy with ${models.length} models`);
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          responseTime: 0,
          error:
            'CORS_ERROR: LM Studio server is blocking cross-origin requests. Try enabling CORS in LM Studio settings or use Veyra desktop app.',
        };
      }
