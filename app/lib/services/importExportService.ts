...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          veyra_user_profile: this._safeGetItem('veyra_user_profile'),
          veyra_settings: this._safeGetItem('veyra_settings'),
          veyra_profile: this._safeGetItem('veyra_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('veyra_viewed_features'),
          developer_mode: this._safeGetItem('veyra_developer_mode'),
        ...
        ui: {
          // Tab configuration
          veyra_tab_configuration: this._safeGetItem('veyra_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('veyra_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('veyra_acknowledged_connection_issue'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('veyra_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    try {
      localStorage.removeItem('veyra_chat_history');
    } catch {
  ...
  private static async _importComprehensiveFormat(data: ImportData): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.veyra_tab_configuration) {
        try {
          this._safeSetItem('veyra_tab_configuration', data.ui.veyra_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'veyra_acknowledged_debug_issues',
        'veyra_acknowledged_connection_issue',
        'error_logs',
        'devonz_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('veyra_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
