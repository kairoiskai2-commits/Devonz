...
const DEFAULT_PROJECTS_DIR = nodePath.join(os.homedir(), '.veyra', 'projects');
...
export class LocalRuntime implements RuntimeProvider {
   ...
   * The host app's own port (Veyra dev server) must not be treated as a
   * project preview server.
   */
  #isExcludedPort(port: number): boolean {
    /*
     * The Veyra app runs on PORT env var (default 5173 in dev, set explicitly
