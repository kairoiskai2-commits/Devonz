...
import { newVeyraShellProcess, newShellProcess } from '~/utils/shell';
...
export class TerminalStore {
  ...
  #veyraTerminal = newVeyraShellProcess();
  ...
  async attachVeyraTerminal(terminal: ITerminal) {
    try {
      ...
      await this.#veyraTerminal.init(runtime, terminal);
    } catch (error: unknown) {
      terminal.write(
        coloredText.red('Failed to spawn veyra shell\n\n') + (error instanceof Error ? error.message : String(error)),
      );
