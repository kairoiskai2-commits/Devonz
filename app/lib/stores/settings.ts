...
/** Providers that work without any API key — auto-enabled on first launch. */
export const FREE_PROVIDERS = ['Pollinations'];
...
const getInitialProviderSettings = (): ProviderSetting => {
  ...
  PROVIDER_LIST.forEach((provider) => {
    ...
    const isFree = FREE_PROVIDERS.includes(provider.name);
    ...
    initialSettings[provider.name] = {
      ...
      settings: {
         ...
         * - Free providers (no API key needed): always enabled by default
         * - Cloud providers WITH a saved API key: enabled
         * - Cloud providers WITHOUT a saved API key: disabled
         */
        enabled: isLocal ? false : (isFree || hasApiKey),
      },
    };
  });
...
const getInitialTabConfiguration = (): TabWindowConfig => {
  ...
  try {
    const saved = localStorage.getItem('veyra_tab_configuration');
...
export const resetTabConfiguration = () => {
  ...
  localStorage.setItem('veyra_tab_configuration', JSON.stringify(defaultConfig));
};
