...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      veyraVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.veyraVersion) {
      body += `- Veyra: ${data.environmentInfo.veyraVersion}\n`;
    }
  ...
  body += '---\n*Submitted via Veyra bug report feature*';
...
async function bugReportAction({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo = context?.cloudflare?.env?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'zebbern/Veyra';
