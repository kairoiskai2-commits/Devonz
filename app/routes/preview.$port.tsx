...
export default function PreviewWindow() {
  ...
  useEffect(() => {
    ...
    /* Use the server-side proxy so the preview works in all environments
     * (Replit, remote servers, etc.) where the browser cannot reach localhost. */
    const url = `/api/preview-proxy/${port}`;
    setPreviewUrl(url);
...
export function ErrorBoundary() {
  ...
  useEffect(() => {
    console.error('[Veyra:PreviewRouteError]', {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-veyra-elements-background-depth-1 text-veyra-elements-textPrimary px-6">
      <div className="max-w-md w-full text-center">
        ...
        <p className="text-sm text-veyra-elements-textSecondary mb-6">{message}</p>
        <button
