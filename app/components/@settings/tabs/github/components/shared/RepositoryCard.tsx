...
export function RepositoryCard({
  ...
  if (variant === 'compact') {
    return (
      <button
        onClick={onSelect}
        className={cn(
          'w-full text-left p-3 rounded-lg border border-veyra-elements-borderColor hover:border-veyra-elements-borderColorActive hover:bg-veyra-elements-background-depth-1 transition-all duration-200',
          className,
        )}
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium text-veyra-elements-textPrimary">{repository.name}</h4>
            {repository.private && <div className="i-ph:lock size-3 text-veyra-elements-textTertiary" />}
            {repository.fork && <div className="i-ph:git-fork size-3 text-veyra-elements-textTertiary" />}
            {repository.archived && <div className="i-ph:archive size-3 text-veyra-elements-textTertiary" />}
          </div>

          <div className="flex items-center gap-3 text-xs text-veyra-elements-textSecondary">
        ...
        {repository.description && (
          <p className="text-xs text-veyra-elements-textSecondary mb-2 line-clamp-2">{repository.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-veyra-elements-textTertiary">
            {repository.language && (
              <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1 text-xs text-veyra-elements-textTertiary">
  ...
  const interactiveProps = onSelect
    ? {
        onClick: onSelect,
        className: cn(
          'group cursor-pointer hover:border-veyra-elements-borderColorActive dark:hover:border-veyra-elements-borderColorActive transition-all duration-200',
          className,
        ),
      }
    : { className };
  ...
  return (
    <Component
      {...interactiveProps}
      className={cn(
        'block p-4 rounded-lg bg-veyra-elements-background-depth-1 dark:bg-veyra-elements-background-depth-1 border border-veyra-elements-borderColor dark:border-veyra-elements-borderColor relative',
    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <div className="i-ph:git-branch size-4 text-veyra-elements-icon-tertiary" />
            <h5
              className={cn(
                'text-sm font-medium text-veyra-elements-textPrimary',
                onSelect && 'group-hover:text-veyra-elements-item-contentAccent transition-colors',
              )}
            >
              {repository.name}
            </h5>
            {repository.fork && (
              <span title="Forked repository">
                <div className="i-ph:git-fork size-3 text-veyra-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title="Archived repository">
                <div className="i-ph:archive size-3 text-veyra-elements-textTertiary" />
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-veyra-elements-textSecondary">
            <span className="flex items-center gap-1" title="Stars">
              <div className="i-ph:star size-3.5 text-veyra-elements-icon-warning" />
              {repository.stargazers_count.toLocaleString()}
            </span>
            <span className="flex items-center gap-1" title="Forks">
              <div className="i-ph:git-fork size-3.5 text-veyra-elements-icon-info" />
              {repository.forks_count.toLocaleString()}
            </span>
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title="Open Issues">
                <div className="i-ph:circle size-3.5 text-veyra-elements-icon-error" />
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title="Pull Requests">
                <div className="i-ph:git-pull-request size-3.5 text-veyra-elements-icon-success" />
        ...
        <div className="space-y-2">
          {repository.description && (
            <p className="text-xs text-veyra-elements-textSecondary line-clamp-2">{repository.description}</p>
          )}

          {/* Repository metrics bar */}
          <div className="flex items-center gap-2 text-xs">
            {repository.license && (
              <span className="px-2 py-0.5 rounded-full bg-veyra-elements-background-depth-2 text-veyra-elements-textTertiary">
            ...
            {repository.fork && (
              <span className="px-2 py-0.5 rounded-full bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent">
                Fork
              </span>
            )}
          </div>
            ...
            {onSelect && (
              <span
                className={cn(
                  ...
                  'group-hover:text-veyra-elements-item-contentAccent',
                )}
              >
