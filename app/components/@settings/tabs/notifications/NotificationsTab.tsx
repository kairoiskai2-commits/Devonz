...
const NotificationsTab = () => {
  ...
  const getNotificationStyle = (level: string, type?: string) => {
    if (type === 'update') {
      return {
        ...
        bg: 'hover:bg-veyra-elements-item-backgroundAccent',
      };
    }
  ...
  const renderNotificationDetails = (details: NotificationDetails) => {
    if (details.type === 'update') {
      return (
        <div className="flex flex-col gap-2">
          <p className="text-sm text-veyra-elements-textSecondary">{details.message}</p>
    ...
    return details.message ? <p className="text-sm text-veyra-elements-textSecondary">{details.message}</p> : null;
  };
  ...
  const filterOptions: { id: FilterType; label: string; icon: string; color: string }[] = [
    ...
    { id: 'system', label: 'System', icon: 'i-ph:gear', color: 'var(--veyra-elements-textTertiary)' },
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {filterOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.id}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-veyra-elements-item-backgroundAccent cursor-pointer transition-colors"
        ...
        >
          <span className="i-ph:trash text-lg text-veyra-elements-textTertiary group-hover:text-veyra-elements-item-contentAccent transition-colors" />
          Clear All
        </button>
      </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-veyra-elements-textPrimary">No Notifications</h3>
              <p className="text-sm text-veyra-elements-textTertiary">You're all caught up!</p>
            </div>
          </motion.div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <h3 className="text-sm font-medium text-veyra-elements-textPrimary">{log.message}</h3>
                  ...
                  <time className="shrink-0 text-xs text-veyra-elements-textTertiary">
