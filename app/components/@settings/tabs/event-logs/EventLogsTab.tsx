...
const logLevelOptions: SelectOption[] = [
  ...
  },
  {
    ...
    label: 'Debug',
    icon: 'i-ph:bug',
    color: 'var(--veyra-elements-textTertiary)',
  },
];
...
const LogEntryItem = ({ log, isExpanded: forceExpanded, use24Hour, showTimestamp }: LogEntryItemProps) => {
  ...
  const style = useMemo(() => {
    ...
    switch (log.level) {
      ...
      case 'debug':
        return {
          icon: 'i-ph:bug',
          color: 'text-veyra-elements-textTertiary',
          bg: 'hover:bg-gray-500/10 dark:hover:bg-gray-500/20',
          badge: 'text-veyra-elements-textTertiary bg-veyra-elements-bg-depth-2',
        };
      default:
        return {
          icon: 'i-ph:info',
  ...
  const renderDetails = (details: Record<string, React.ReactNode>) => {
    if (log.category === 'provider') {
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-veyra-elements-textTertiary">
          ...
          {details.prompt && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-veyra-elements-textPrimary">Prompt:</div>
              <pre
                className="text-xs text-veyra-elements-textSecondary rounded p-2 whitespace-pre-wrap border-l-2 border-veyra-elements-borderColor"
          ...
          {details.response && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-veyra-elements-textPrimary">Response:</div>
              <pre
                className="text-xs text-veyra-elements-textSecondary rounded p-2 whitespace-pre-wrap border-l-2 border-veyra-elements-borderColor"
                style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}
              >
    ...
    if (log.category === 'api') {
      return (
        <div className="flex flex-col gap-2">
          ...
          <div className="text-xs text-veyra-elements-textSecondary break-all">{details.url}</div>
          {details.request && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-veyra-elements-textPrimary">Request:</div>
              <pre
                className="text-xs text-veyra-elements-textSecondary rounded p-2 whitespace-pre-wrap border-l-2 border-veyra-elements-borderColor"
                style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}
              >
    ...
    return (
      <div
        className="rounded-lg p-3 border-l-2 border-veyra-elements-borderColorActive"
        style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}
      >
          ...
          {entries.map(([key, value]) => {
            ...
            return (
              <React.Fragment key={key}>
                <span className="text-xs text-veyra-elements-textTertiary font-medium whitespace-nowrap">{key}</span>
                {isObject ? (
                  <pre className="text-xs text-veyra-elements-textSecondary whitespace-pre-wrap break-all">
                    {JSON.stringify(value, null, 2)}
                  </pre>
                ) : (
                  <span className="text-xs text-veyra-elements-textSecondary break-all">{String(value)}</span>
                )}
              </React.Fragment>
            );
          })}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-1">
            <div className="text-sm font-medium text-veyra-elements-textPrimary">{log.message}</div>
            {log.details && (
              <>
                <button
                  onClick={() => setLocalExpanded(!localExpanded)}
                  className="inline-flex items-center gap-1 text-xs text-veyra-elements-item-contentAccent hover:underline transition-colors w-fit bg-transparent border-none p-0 cursor-pointer"
                >
                  <span
                    className={cn(
            ...
            <div className="flex items-center gap-2">
              ...
              {log.category && (
                <div className="px-2 py-0.5 rounded-full text-xs text-veyra-elements-textTertiary border border-veyra-elements-borderColor">
                  {log.category}
                </div>
              )}
            </div>
        ...
        {showTimestamp && <time className="shrink-0 text-xs text-veyra-elements-textTertiary">{timestamp}</time>}
      </div>
    </motion.div>
  );
};
...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `veyra-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `veyra-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = async () => {
    try {
      ...
      // Add subtitle with veyra.dev
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('veyra.dev - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by veyra.dev', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`veyra-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `veyra-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      ...
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        <button
          ...
          className={cn(
            ...
            'bg-transparent',
            'border border-veyra-elements-borderColor',
            'hover:bg-veyra-elements-item-backgroundAccent',
            'transition-all duration-200',
          )}
        >
          <span className="i-ph:download text-lg text-veyra-elements-textTertiary group-hover:text-veyra-elements-item-contentAccent transition-colors" />
          Export
        </button>
              ...
              {exportFormats.map((format) => (
                <button
                  ...
                  className={cn(
                    ...
                    'bg-veyra-elements-bg-depth-1',
                    'border border-veyra-elements-borderColor',
                    'hover:bg-veyra-elements-bg-depth-2',
                    'hover:border-veyra-elements-borderColorActive',
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-veyra-elements-textSecondary mt-0.5">
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
            <DropdownMenu.Trigger asChild>
              ...
              >
                ...
                <span className="i-ph:caret-down text-lg text-veyra-elements-textTertiary" />
              </button>
            </DropdownMenu.Trigger>

            <DropdownMenu.Portal>
              <DropdownMenu.Content
                className="min-w-[200px] rounded-lg shadow-lg py-1 z-[250] animate-in fade-in-0 zoom-in-95 border border-veyra-elements-borderColor"
                style={{ backgroundColor: 'var(--veyra-elements-bg-depth-1)' }}
                sideOffset={5}
                align="start"
                side="bottom"
              >
                {logLevelOptions.map((option) => (
                  <DropdownMenu.Item
                    key={option.value}
                    className="group flex items-center px-4 py-2.5 text-sm text-veyra-elements-textSecondary hover:bg-veyra-elements-item-backgroundAccent cursor-pointer transition-colors"
                  ...
                  >
                    <div className="mr-3 flex h-5 w-5 items-center justify-center">
                      <div
                        className={cn(
                          option.icon,
                          'text-lg group-hover:text-veyra-elements-item-contentAccent transition-colors',
                        )}
                        style={{ color: option.color }}
                      />
                    </div>
                    <span className="group-hover:text-veyra-elements-item-contentAccent transition-colors">
                      {option.label}
                    </span>
                  </DropdownMenu.Item>
                ))}
          ...
          <div className="relative flex-1">
            ...
            <div className="absolute left-3 top-1/2 -translate-y-1/2">
              <div className="i-ph:magnifying-glass text-lg text-veyra-elements-textTertiary" />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Switch
                ...
                className="data-[state=checked]:bg-veyra-elements-item-contentAccent"
              />
              <span className="text-sm text-veyra-elements-textTertiary">Timestamps</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                className="data-[state=checked]:bg-veyra-elements-item-contentAccent"
              />
              <span className="text-sm text-veyra-elements-textTertiary">24h</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                className="data-[state=checked]:bg-veyra-elements-item-contentAccent"
              />
              <span className="text-sm text-veyra-elements-textTertiary">Auto Expand</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className="i-ph:arrows-clockwise text-lg text-veyra-elements-textTertiary group-hover:text-veyra-elements-item-contentAccent transition-colors" />
              Refresh
            </button>
      ...
      <div className="flex flex-col gap-2">
        {filteredLogs.length === 0 ? (
          ...
          >
            <span className="i-ph:clipboard-text text-4xl text-veyra-elements-textTertiary" />
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-veyra-elements-textPrimary">No Logs Found</h3>
              <p className="text-sm text-veyra-elements-textSecondary">Try adjusting your search or filters</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => (
