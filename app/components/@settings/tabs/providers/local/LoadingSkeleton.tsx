  ...
  return (
    <div className={cn('space-y-2', className)}>
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className={cn('bg-veyra-elements-background-depth-3 rounded', height, 'animate-pulse')}
  ...
  return (
    <div
      className={cn(
        ...
        'border-veyra-elements-borderColor',
        className,
      )}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 flex-1">
          <div className="w-3 h-3 rounded-full bg-veyra-elements-textTertiary animate-pulse" />
          <div className="space-y-2 flex-1">
            <LoadingSkeleton height="h-5" lines={1} className="w-3/4" />
            <LoadingSkeleton height="h-3" lines={1} className="w-1/2" />
          </div>
        </div>
        <div className="w-4 h-4 bg-veyra-elements-textTertiary rounded animate-pulse" />
      </div>
    </div>
  );
}
