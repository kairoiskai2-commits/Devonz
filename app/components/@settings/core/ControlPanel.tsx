...
const TabLoadingFallback = () => (
  <div className="flex items-center justify-center h-64">
    <div className="animate-spin w-8 h-8 border-2 border-veyra-elements-item-contentAccent border-t-transparent rounded-full" />
  </div>
);
      ...
      <RadixDialog.Portal>
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          <RadixDialog.Content
            ...
            className="dark relative z-[101] w-[1000px] h-[80vh] rounded-xl shadow-2xl border border-veyra-elements-borderColor flex overflow-hidden"
            style={{ backgroundColor: 'var(--veyra-elements-bg-depth-1)' }}
          >
            {/* Sidebar */}
            <div
              className="w-52 border-r border-veyra-elements-borderColor flex flex-col"
              style={{ backgroundColor: 'var(--veyra-elements-bg-depth-1)' }}
            >
              {/* Header */}
              <div className="px-4 py-4 border-b border-veyra-elements-borderColor">
                <h2 className="text-sm font-semibold text-veyra-elements-textPrimary">Settings</h2>
              </div>
                ...
                {categorizedTabs.map((category, catIndex) => (
                  <div key={category.id} className={cn(catIndex > 0 ? 'mt-3' : '')}>
                    ...
                    <div className="flex items-center gap-2 px-4 py-1.5 mb-0.5">
                      <div className={cn(category.icon, 'w-3.5 h-3.5 text-veyra-elements-textTertiary')} />
                      <span className="text-[11px] font-medium uppercase tracking-wider text-veyra-elements-textTertiary">
                        {category.label}
                      </span>
                    </div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-veyra-elements-focus focus-visible:ring-inset',
                            isActive
                              ? 'text-veyra-elements-textPrimary border-l-2 border-veyra-elements-item-contentAccent'
                              : 'text-veyra-elements-textSecondary hover:text-veyra-elements-textPrimary border-l-2 border-transparent',
                          )}
                          style={{
                            backgroundColor: isActive ? 'var(--veyra-elements-bg-depth-3)' : 'transparent',
                        ...
                        >
                          ...
                          {hasUpdate && (
                            <span className="ml-auto w-2 h-2 rounded-full bg-veyra-elements-item-contentAccent shrink-0" />
                          )}
                          {BETA_TABS.has(tabId) && (
                            <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent shrink-0">
                              BETA
                            </span>
                          )}
                        </button>
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-veyra-elements-borderColor">
                <DialogTitle className="text-sm font-semibold text-veyra-elements-textPrimary">
                  {activeTab ? TAB_LABELS[activeTab] : 'Settings'}
                </DialogTitle>
                <button
                  onClick={handleClose}
                  aria-label="Close settings"
                  className="p-1.5 rounded transition-colors hover:bg-veyra-elements-bg-depth-4"
                  style={{ backgroundColor: 'var(--veyra-elements-bg-depth-3)' }}
                >
                  <div className="i-ph:x w-4 h-4 text-veyra-elements-textSecondary" />
                </button>
              </div>
              ...
              >
                {activeTab ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full gap-6">
                    <div className="i-ph:gear w-12 h-12 text-veyra-elements-textTertiary" />
                    <div className="text-center">
                      <p className="text-sm text-veyra-elements-textSecondary mb-1">
                        ...
                        cat.tabs.slice(0, 2).map((tabId) => {
                          ...
                          return (
                            <button
                              key={tabId}
                              onClick={() => handleTabClick(tabId)}
                              className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-veyra-elements-textSecondary transition-colors hover:text-veyra-elements-textPrimary"
                              style={{ backgroundColor: 'var(--veyra-elements-bg-depth-3)' }}
                            >
