...
export const BoxModelEditor = memo(({ boxModel, onValueChange }: BoxModelEditorProps) => {
  ...
  if (!boxModel) {
    return (
      <div className="text-veyra-elements-textSecondary text-xs text-center py-4">No box model data available</div>
    );
  }
  ...
  const renderEditableValue = (field: string, value: number, textColor: string) => {
    ...
    if (editingField === field) {
      return (
        <input
          ...
          className="w-10 h-4 text-center text-[10px] bg-veyra-elements-background-depth-4 border border-veyra-elements-borderColor rounded focus:outline-none focus:border-accent-400"
    ...
    return (
      <button
        ...
        className={`text-[10px] font-mono hover:bg-veyra-elements-background-depth-4 px-1 rounded cursor-pointer transition-colors ${textColor}`}
    ...
    <div className="space-y-3" role="region" aria-label="Box model visualization">
      ...
      <div className="flex flex-wrap gap-3 text-[10px] justify-center pt-1">
        <div className="flex items-center gap-1">
          ...
          <span className="text-veyra-elements-textSecondary">Margin</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 bg-yellow-500/30 border border-yellow-500/50 rounded" />
          <span className="text-veyra-elements-textSecondary">Border</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 bg-green-500/30 border border-green-500/50 rounded" />
          <span className="text-veyra-elements-textSecondary">Padding</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 bg-blue-500/30 border border-blue-500/50 rounded" />
          <span className="text-veyra-elements-textSecondary">Content</span>
        </div>
      </div>

      {/* Additional Info */}
      <div className="border-t border-veyra-elements-borderColor pt-2">
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="bg-veyra-elements-background-depth-3 rounded p-2 border border-veyra-elements-borderColor">
            <span className="text-veyra-elements-textSecondary block text-[10px]">Box Sizing</span>
            <span className="text-veyra-elements-textPrimary font-mono text-[11px]">{boxModel.boxSizing}</span>
          </div>
          <div className="bg-veyra-elements-background-depth-3 rounded p-2 border border-veyra-elements-borderColor">
            <span className="text-veyra-elements-textSecondary block text-[10px]">Border Style</span>
            <span className="text-veyra-elements-textPrimary font-mono text-[11px]">
      ...
      {/* Tip */}
      <p className="text-veyra-elements-textTertiary text-[10px] text-center italic">
