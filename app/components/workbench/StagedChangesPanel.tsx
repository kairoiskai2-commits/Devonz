...
function getChangeTypeColor(type: ChangeType): string {
  switch (type) {
    ...
    case 'delete':
      return 'text-red-400';
    default:
      return 'text-veyra-elements-textSecondary';
  }
}
...
const ChangeItem = memo(({ change, onAccept, onReject, onPreview }: ChangeItemProps) => {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      exit="exit"
      className="flex items-center gap-2 px-3 py-2 hover:bg-veyra-elements-background-depth-2 rounded-md group cursor-pointer"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-veyra-elements-textPrimary truncate">{name}</span>
          <span className={cn('w-4 h-4', getChangeTypeIcon(change.type), getChangeTypeColor(change.type))} />
        </div>
        {dir && <span className="text-xs text-veyra-elements-textTertiary truncate block">{dir}</span>}
      </div>
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <IconButton
          ...
          title="Preview diff"
          className="text-veyra-elements-textTertiary hover:text-veyra-elements-textSecondary"
        />
        <IconButton
          icon="i-ph:check"
          size="sm"
  ...
  return (
    <div className="mb-2">
      <div className="flex items-center gap-2 px-3 py-1">
        ...
        <span className="text-xs font-medium text-veyra-elements-textSecondary uppercase tracking-wide">{title}</span>
        <span className="text-xs text-veyra-elements-textTertiary">({changes.length})</span>
      </div>
...
export const StagedChangesPanel = memo(() => {
  ...
  const executePendingCommands = useCallback(async () => {
    ...
    const shell = workbenchStore.veyraTerminal;
  ...
  return (
    <Collapsible.Root open={isOpen} onOpenChange={setIsOpen}>
      <div
        className="border-b border-veyra-elements-borderColor"
        style={{ background: 'var(--veyra-elements-bg-depth-1)' }}
      >
        {/* Header */}
        <Collapsible.Trigger asChild>
          <button className="w-full flex items-center justify-between px-4 py-3 bg-veyra-elements-background-depth-2 hover:bg-veyra-elements-background-depth-3 transition-colors">
            <div className="flex items-center gap-3">
              <motion.span
                className="i-ph:caret-right w-4 h-4 text-veyra-elements-textSecondary"
                animate={{ rotate: isOpen ? 90 : 0 }}
                transition={{ duration: 0.15 }}
              />
              <span className="i-ph:git-diff w-5 h-5 text-veyra-elements-textSecondary" />
              <span className="text-sm font-medium text-veyra-elements-textPrimary">Pending Changes</span>
        ...
        <Collapsible.Content forceMount>
          ...
          >
            ...
            <div
              ...
              style={{ scrollbarColor: 'var(--veyra-elements-borderColor) var(--veyra-elements-bg-depth-2)' }}
            >
              <ChangeGroup
                title="New Files"
              ...
              {hasCmds && (
                <div className="mb-2 mt-2 border-t border-veyra-elements-borderColor pt-2">
                  <div className="flex items-center gap-2 px-3 py-1">
                    <span className="i-ph:terminal w-4 h-4 text-blue-400" />
                    <span className="text-xs font-medium text-veyra-elements-textSecondary uppercase tracking-wide">
                      Queued Commands
                    </span>
                    <span className="text-xs text-veyra-elements-textTertiary">({cmdCount})</span>
                  </div>
                  {pendingCmds.map((cmd) => (
                    <div
                      key={cmd.id}
                      className="flex items-center gap-2 px-3 py-2 hover:bg-veyra-elements-background-depth-2 rounded-md"
                    >
                      <span
                        className={cn(
                      ...
                      />
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-mono text-veyra-elements-textPrimary truncate block">
                          {cmd.command.length > 50 ? `${cmd.command.substring(0, 50)}...` : cmd.command}
                        </span>
                        <span className="text-xs text-veyra-elements-textTertiary">{cmd.type} command</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Footer actions */}
            <div className="flex items-center justify-between px-4 py-3 border-t border-veyra-elements-borderColor bg-veyra-elements-background-depth-2">
              <span className="text-xs text-veyra-elements-textTertiary">
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isPreviewMode
                      ...
                      : 'bg-veyra-elements-button-secondary-background hover:bg-veyra-elements-button-secondary-backgroundHover text-veyra-elements-button-secondary-text',
                  )}
                >
