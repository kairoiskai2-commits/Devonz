...
const taskStatusColors: Record<PlanTask['status'], string> = {
  'not-started': 'text-veyra-elements-textSecondary',
  'in-progress': 'text-blue-500',
  completed: 'text-green-500',
};

const subTaskStatusColors: Record<SubTaskStatus, string> = {
  pending: 'text-veyra-elements-textSecondary',
  'in-progress': 'text-blue-500',
  done: 'text-green-500',
  failed: 'text-red-500',
};
...
const StatusIcon = memo(({ status }: { status: PlanTask['status'] }) => {
  switch (status) {
    ...
    default:
      return <div className="w-5 h-5 rounded-full border-2 border-veyra-elements-borderColor bg-transparent" />;
  }
});
...
const SubTaskStatusIcon = memo(({ status }: { status: SubTaskStatus }) => {
  switch (status) {
    ...
    default:
      return <div className="w-4 h-4 rounded-full border border-veyra-elements-borderColor bg-transparent" />;
  }
});
...
const DependencyChips = memo(({ dependsOn, allTasks }: { dependsOn: string[]; allTasks: PlanTask[] }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 mt-1">
      <div className="i-ph:arrow-bend-down-right text-xs text-veyra-elements-textSecondary" />
          ...
          <span
            ...
            className={cn(
              ...
              isDone
                ...
                : 'bg-veyra-elements-background-depth-3 text-veyra-elements-textSecondary border-veyra-elements-borderColor',
            )}
          >
            {depId}
          </span>
    ...
    <motion.div
      ...
      className={cn(
        ...
        task.status === 'in-progress'
          ...
          : 'bg-veyra-elements-background-depth-2 border border-transparent',
      )}
    >
      {/* Main task row */}
      <div className="flex items-start gap-3 p-3">
        ...
        {hasSubTasks ? (
          ...
          >
            <div
              className={cn(
                'i-ph:caret-right text-sm text-veyra-elements-textSecondary transition-transform',
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {completionPct !== null && (
              <span className="text-xs text-veyra-elements-textSecondary ml-auto flex-shrink-0">{completionPct}%</span>
            )}
          </div>

          {task.description && (
            <div className="text-xs text-veyra-elements-textSecondary mt-1 line-clamp-2">{task.description}</div>
          )}
          ...
          {task.fileActions && task.fileActions.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {task.fileActions.map((file, i) => (
                <span
                  key={`${file}-${i}`}
                  className="text-xs px-1.5 py-0.5 rounded bg-veyra-elements-background-depth-3 text-veyra-elements-textSecondary font-mono"
                >
                  {file}
                </span>
              ))}
        ...
        <AnimatePresence>
          {subTasksOpen && (
            <motion.div
              ...
              className="border-t border-veyra-elements-borderColor/50 pb-2"
            >
...
export const Plan = memo(({ className }: PlanProps) => {
  ...
  return (
    <div className={cn('border-b border-veyra-elements-borderColor', className)}>
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger asChild>
          <button
            className={cn(
              'w-full flex items-center justify-between p-4',
              'bg-veyra-elements-background-depth-1 hover:bg-veyra-elements-background-depth-2',
              'transition-colors cursor-pointer',
            )}
          >
            <div className="flex items-center gap-3">
              <div className="i-ph:list-checks-fill text-xl text-veyra-elements-textPrimary" />
              <div className="text-left">
                <h3 className="font-semibold text-veyra-elements-textPrimary">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-veyra-elements-textSecondary">{progress}%</span>
              </div>

              {/* Chevron */}
              <div
                className={cn(
                  'i-ph:caret-down text-veyra-elements-textSecondary transition-transform',
                  isOpen ? 'rotate-180' : '',
                )}
              />
            </div>
        ...
        <CollapsibleContent>
          <AnimatePresence>
            ...
            >
              ...
              {/* Approval actions */}
              <div className="pt-3 border-t border-veyra-elements-borderColor">
