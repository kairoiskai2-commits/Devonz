...
export const Search = memo(() => {
  ...
  return (
    <div className="flex flex-col h-full bg-veyra-elements-background-depth-2">
      {/* Search Bar */}
      <div className="flex items-center py-3 px-3">
        <div className="relative flex-1">
          <input
            ...
            className="w-full px-2 py-1 rounded-md bg-veyra-elements-background-depth-3 text-veyra-elements-textPrimary placeholder-veyra-elements-textTertiary focus:outline-none focus-visible:ring-2 focus-visible:ring-veyra-elements-focus transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto py-2">
        {isSearching && (
          <div className="flex items-center justify-center h-32 text-veyra-elements-textTertiary">
        ...
        {!isSearching &&
          Object.keys(groupedResults).map((file) => (
            <div key={file} className="mb-2">
              <button
                className="flex gap-2 items-center w-full text-left py-1 px-2 text-veyra-elements-textSecondary bg-transparent hover:bg-veyra-elements-background-depth-3 group"
              ...
              >
                <span
                  className=" i-ph:caret-down-thin w-3 h-3 text-veyra-elements-textSecondary transition-transform"
                  style={{ transform: expandedFiles[file] ? 'rotate(180deg)' : undefined }}
                />
                <span className="font-normal text-sm">{file.split('/').pop()}</span>
                <span className="h-5.5 w-5.5 flex items-center justify-center text-xs ml-auto bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent rounded-full">
                  ...
                  {groupedResults[file].map((match, idx) => {
                    ...
                    return (
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        className="hover:bg-veyra-elements-background-depth-3 cursor-pointer transition-colors pl-6 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-veyra-elements-focus"
                      ...
                      >
                        <pre className="font-mono text-xs text-veyra-elements-textTertiary truncate">
                          {!isStart && <span>...</span>}
                          {previewText.slice(0, matchStart)}
                          <span className="bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent rounded px-1">
