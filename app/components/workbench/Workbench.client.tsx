...
import { useAutoDeployOnComplete } from '~/lib/hooks/useAutoDeployOnComplete';
...
export const Workbench = memo(({ chatStarted, isStreaming, setSelectedElement, width, fullWidth }: WorkspaceProps) => {
  ...
  // Auto-deploy when AI generation completes (only if VITE_AUTO_DEPLOY_ON_COMPLETE=true)
  useAutoDeployOnComplete({ chatId: currentChatId ?? null });
  ...
  return (
    ...
    showWorkbench && (
      ...
      >
        <div
          className={cn('h-full flex flex-col bg-veyra-elements-background-depth-2 overflow-hidden', {
            'border-l border-veyra-elements-borderColor': !fullWidth,
          })}
        >
          <div className="flex items-center px-3 py-2.5 border-b border-veyra-elements-borderColor gap-2 bg-veyra-elements-background-depth-2/80">
            <button
              aria-label="Toggle file explorer"
              className={`${showChat ? 'i-ph:sidebar-simple-fill' : 'i-ph:sidebar-simple'} text-lg text-veyra-elements-textSecondary hover:text-veyra-elements-textPrimary transition-colors mr-1`}
            ...
            {selectedView === 'code' && (
              <div className="flex overflow-y-auto">
                ...
                {/* Sync Button */}
                <div className="flex border border-veyra-elements-borderColor rounded-lg overflow-hidden ml-1">
                  <DropdownMenu.Root>
                    <DropdownMenu.Trigger
                      disabled={isSyncing || streaming}
                      className="rounded-lg items-center justify-center [&:is(:disabled,.disabled)]:cursor-not-allowed [&:is(:disabled,.disabled)]:opacity-60 px-3 py-1.5 text-xs bg-veyra-elements-background-depth-3 text-veyra-elements-textPrimary border border-veyra-elements-borderColor hover:bg-veyra-elements-background-depth-4 [&:not(:disabled,.disabled)]:hover:text-accent-400 outline-accent-500 flex gap-1.5 transition-colors"
                    >
                    ...
                    <DropdownMenu.Portal>
                      ...
                      >
                        <DropdownMenu.Item
                          className={cn(
                            'cursor-pointer flex items-center w-full px-4 py-2 text-sm text-veyra-elements-textPrimary hover:bg-veyra-elements-item-backgroundActive gap-2 rounded-md group relative',
                          )}
                ...
                <div className="flex border border-veyra-elements-borderColor rounded-md overflow-hidden ml-1">
                  <button
                    onClick={() => {
                      workbenchStore.toggleTerminal(!workbenchStore.showTerminal.get());
                    }}
                    className="rounded-md items-center justify-center [&:is(:disabled,.disabled)]:cursor-not-allowed [&:is(:disabled,.disabled)]:opacity-60 px-3 py-1.5 text-xs bg-veyra-elements-background-depth-3 text-veyra-elements-textPrimary border border-veyra-elements-borderColor hover:bg-veyra-elements-background-depth-4 [&:not(:disabled,.disabled)]:hover:text-accent-400 outline-accent-500 flex gap-1.7"
                  >
