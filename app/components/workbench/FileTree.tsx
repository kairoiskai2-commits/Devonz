...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-veyra-elements-textPrimary cursor-pointer ws-nowrap text-veyra-elements-item-contentDefault hover:text-veyra-elements-item-contentActive hover:bg-veyra-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-veyra-elements-background-depth-4 border border-veyra-elements-item-contentAccent py-0.5 text-veyra-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-veyra-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        spellCheck={false}
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-veyra-elements-textPrimary placeholder:text-veyra-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={cn('relative', {
              'bg-veyra-elements-background-depth-2 border border-dashed border-veyra-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-veyra-elements-borderColor rounded-md z-context-menu bg-veyra-elements-background-depth-1 dark:bg-veyra-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-veyra-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-veyra-elements-borderColor">
              {!isFolder ? (
                <>
...
function Folder({ folder, collapsed, selected = false, onCopyPath, onCopyRelativePath, onClick }: FolderProps) {
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={cn('group', {
          'bg-transparent text-veyra-elements-item-contentDefault hover:text-veyra-elements-item-contentActive hover:bg-veyra-elements-item-backgroundActive':
            !selected,
          'bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
        iconClasses={cn({
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={cn('group', {
          'bg-transparent hover:bg-veyra-elements-item-backgroundActive text-veyra-elements-item-contentDefault':
            !selected,
          'bg-veyra-elements-item-backgroundAccent text-veyra-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={cn('i-ph:file-duotone scale-98', {
          'group-hover:text-veyra-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
