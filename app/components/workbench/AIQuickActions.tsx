...
export const AiQuickActions = ({ selectedElement, onAIAction }: AiQuickActionsProps) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {quickActions.map((action) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-lg bg-veyra-elements-bg-depth-3 border border-veyra-elements-borderColor hover:border-accent-500/50 hover:bg-veyra-elements-background-depth-4 transition-all group ${clickedActionId === action.id ? 'animate-pulse opacity-70 pointer-events-none' : ''}`}
            title={action.description}
          >
            <div
              className={`${action.icon} w-4 h-4 text-veyra-elements-textSecondary group-hover:text-accent-400 transition-colors`}
            />
            <span className="text-[10px] text-veyra-elements-textSecondary group-hover:text-veyra-elements-textPrimary">
              {action.label}
            </span>
          </button>
        ))}
      ...
      <p className="text-[10px] text-veyra-elements-textTertiary text-center italic">
