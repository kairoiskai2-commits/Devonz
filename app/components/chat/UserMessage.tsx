    ...
    return (
      <div className="overflow-hidden flex flex-col gap-3 items-end w-full min-w-0">
        <div className="flex flex-row items-center gap-2 self-end">
          {profile?.avatar || profile?.username ? (
            <div className="flex items-center gap-2">
              <span className="text-veyra-elements-textSecondary text-sm">
                {profile?.username ? profile.username : 'You'}
              </span>
              <img
                src={profile.avatar}
                alt={profile?.username || 'User'}
                className="w-6 h-6 object-cover rounded-full ring-1 ring-veyra-elements-borderColor"
                loading="eager"
                decoding="sync"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-veyra-elements-textSecondary text-sm">You</span>
        ...
        <div className="flex flex-col gap-3 max-w-[85%] ml-auto overflow-hidden">
          {textContent && (
            <div className="text-veyra-elements-textPrimary text-sm leading-relaxed min-w-0">
            ...
            <div className="flex flex-wrap gap-2">
              {images.map((item, index) => (
                <img
                  ...
                  alt={`Image ${index + 1}`}
                  className="max-w-full h-auto rounded-lg border border-veyra-elements-borderColor"
  ...
  return (
    <div className="flex flex-col items-end gap-3 w-full min-w-0">
      <div className="flex items-center gap-2">
        {profile?.avatar ? (
          <>
            <span className="text-veyra-elements-textSecondary text-sm">{profile?.username || 'You'}</span>
            <img
              src={profile.avatar}
              alt={profile?.username || 'User'}
              className="w-6 h-6 object-cover rounded-full ring-1 ring-veyra-elements-borderColor"
        ...
        {images.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3">
            {images.map((item, index) => (
              <div
                key={index}
                className="relative rounded-lg border border-veyra-elements-borderColor overflow-hidden"
              >
                <img
                  src={`data:${item.mimeType};base64,${item.data}`}
                  alt={`Image ${index + 1}`}
