...
function BranchIndicator({ parentChatId }: { parentChatId: string }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 mb-2 rounded-md bg-veyra-elements-background-depth-2 border border-veyra-elements-borderColor text-xs">
      <div className="i-ph:git-branch text-veyra-elements-textTertiary w-4 h-4 shrink-0" />
      <span className="text-veyra-elements-textSecondary">Branched from</span>
      <button
        onClick={() => navigate(`/chat/${parentChatId}`)}
        className="text-veyra-elements-item-contentAccent hover:underline truncate max-w-48"
      >
        parent conversation
      </button>
    </div>
    ...
    <div className="relative mt-1">
      <button
        ...
        className={cn(
          ...
          'text-veyra-elements-textTertiary hover:text-veyra-elements-textSecondary',
          'hover:bg-veyra-elements-background-depth-2',
        )}
      >
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-40 min-w-56 rounded-lg border border-veyra-elements-borderColor bg-veyra-elements-background-depth-2 shadow-lg overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-3">
              <div className="i-svg-spinners:90-ring-with-bg text-veyra-elements-textTertiary" />
            </div>
          ) : branches.length === 0 ? (
            <div className="px-3 py-2 text-xs text-veyra-elements-textTertiary">No branches found</div>
          ) : (
            branches.map((branch) => (
              <button
                key={branch.branchId}
                ...
                className="w-full text-left px-3 py-2 text-xs hover:bg-veyra-elements-background-depth-3 transition-colors flex items-center gap-2"
              >
                <div className="i-ph:git-branch w-3.5 h-3.5 text-veyra-elements-textTertiary shrink-0" />
                <div className="flex flex-col min-w-0">
                  <span className="text-veyra-elements-textPrimary truncate">{branch.label}</span>
...
function AgentPhaseIndicator({ phase }: { phase: string }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2 px-3 my-1 rounded-md bg-veyra-elements-background-depth-2 border border-veyra-elements-borderColor">
      <div className="i-svg-spinners:90-ring-with-bg text-veyra-elements-textTertiary w-3.5 h-3.5" />
      <div className={cn('w-4 h-4', iconClass)} />
      <span className={cn('text-xs font-medium px-1.5 py-0.5 rounded-md', colorClass)}>{label}</span>
      <span className="text-xs text-veyra-elements-textTertiary">in progress</span>
    </div>
  );
}
...
export function Messages(props: MessagesProps) {
  ...
  useEffect(() => {
    ...
    if (currentUrlId) {
      try {
        const stored = localStorage.getItem(`veyra_branch_parent_${currentUrlId}`);
    ...
    <div id={id} className={props.className} ref={ref}>
      ...
      <AnimatePresence>
        {isStreaming && (
          ...
          >
            <div className="i-svg-spinners:3-dots-fade text-xl text-veyra-elements-item-contentAccent" />
            <motion.span
              animate={{ opacity: [0.5, 1, 0.5] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
              className="text-sm text-veyra-elements-textSecondary"
            >
              Generating...
            </motion.span>
          </motion.div>
