...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-veyra-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-veyra-elements-background-depth-2 hover:bg-veyra-elements-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Collapse details' : 'Expand details'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-veyra-elements-textSecondary hover:text-veyra-elements-textPrimary transition-colors"></div>
          </div>
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-veyra-elements-textPrimary font-medium leading-5 text-sm">
              Tool Invocations{' '}
              {hasToolResults && (
                <span className="w-full w-full text-veyra-elements-textSecondary text-xs mt-0.5">
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-veyra-elements-artifacts-background hover:bg-veyra-elements-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-veyra-elements-textSecondary hover:text-veyra-elements-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-veyra-elements-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-veyra-elements-background-depth-2">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-veyra-elements-actions-background">
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 text-xs mb-1">
        {isErrorResult ? (
          <div className="text-lg text-veyra-elements-icon-error">
            <div className="i-ph:x"></div>
          </div>
        ) : (
          <div className="text-lg text-veyra-elements-icon-success">
            <div className="i-ph:check"></div>
          </div>
        )}
        <div className="text-veyra-elements-textSecondary text-xs">Server:</div>
        <div className="text-veyra-elements-textPrimary font-semibold">{annotation?.serverName}</div>
      </div>

      <div className="ml-6 mb-2">
        <div className="text-veyra-elements-textSecondary text-xs mb-1">
          Tool: <span className="text-veyra-elements-textPrimary font-semibold">{toolName}</span>
        </div>
        <div className="text-veyra-elements-textSecondary text-xs mb-1">
          Description:{' '}
          <span className="text-veyra-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
        </div>
        <div className="text-veyra-elements-textSecondary text-xs mb-1">Parameters:</div>
        <div className="bg-veyra-elements-bg-depth-1 p-3 rounded-md">
        ...
        <div className="flex items-center justify-between mt-3 mb-1">
          <div className="flex items-center gap-2">
            <div className="text-veyra-elements-textSecondary text-xs">
              Result
              {viewMode === 'raw' && lineCount > 1 && (
                <span className="ml-1.5 text-veyra-elements-textTertiary">({lineCount} lines)</span>
              )}
            </div>

            {/* View mode toggle — only show when formatted content is available */}
            {hasFormattedContent && (
              <div className="flex items-center rounded-md overflow-hidden border border-veyra-elements-borderColor">
                <button
                  onClick={() => {
                  ...
                  className={cn(
                    ...
                    viewMode === 'formatted'
                      ...
                      : 'text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary',
                  )}
          ...
          <button
            ...
            className={cn(
              ...
              copied ? 'text-green-400' : 'text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary',
            )}
        ...
        <div className="bg-veyra-elements-bg-depth-1 p-3 rounded-md relative">
          <div
          ...
          {shouldCollapse && (
            <button
              ...
              className="w-full mt-1 py-1 text-xs text-center text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary transition-colors"
            >
              {isExpanded ? (
...
const ToolCallsList = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolCallsListProps) => {
  ...
  useEffect(() => {
    toolInvocations.forEach((inv) => {
      ...
      const isAgentAutoApproved = serverName === 'veyra-agent' && shouldAutoApproveAgentTool(toolName, agentSettings);
      ...
      <ul className="list-none space-y-4">
        {toolInvocations.map((tool, index) => {
          ...
          const isAutoApproving =
            ...
            (serverName === 'veyra-agent' && shouldAutoApproveAgentTool(toolName, agentSettings));
          ...
          return (
            <motion.li
              ...
              transition={{ duration: 0.2, ease: cubicEasingFn }}
            >
              <div className="bg-veyra-elements-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  <div className="flex flex-col items-center ">
                    <span className="mr-auto font-light font-normal text-md text-veyra-elements-textPrimary rounded-md">
                      {toolName}
                    </span>
                    <span className="text-xs text-veyra-elements-textSecondary font-light break-words max-w-64">
                      ...
                      <>
                        ...
                        <button
                          className={cn(
                            ...
                            'bg-veyra-elements-background-depth-2 border border-veyra-elements-borderColor',
                            'text-accent-500 hover:text-veyra-elements-textPrimary',
