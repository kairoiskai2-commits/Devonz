...
export function SupabaseConnection() {
  ...
  return (
    <div className="relative">
      <div className="flex border border-veyra-elements-borderColor rounded-md overflow-hidden mr-2 text-sm">
        <Button
          active
          disabled={connecting}
          onClick={() => setIsDialogOpen(!isDialogOpen)}
          className="hover:bg-veyra-elements-item-backgroundActive !text-white flex items-center gap-2"
        >
          <img
            className="w-4 h-4"
            height="20"
          ...
          <Dialog className="max-w-[520px] p-6">
            {!isConnected ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 p-3 bg-veyra-elements-bg-depth-2 rounded-lg">
                  <div>
                    <h4 className="text-sm font-medium text-veyra-elements-textPrimary">{supabaseConn.user?.email}</h4>
                    <p className="text-xs text-veyra-elements-textSecondary">Role: {supabaseConn.user?.role}</p>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <button
                        ...
                        className="bg-transparent text-left text-sm font-medium text-veyra-elements-textPrimary flex items-center gap-2"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-2 py-1 rounded-md text-xs bg-veyra-elements-bg-depth-3 text-veyra-elements-textSecondary hover:bg-veyra-elements-bg-depth-4 flex items-center gap-1"
                    ...
                    {isProjectsExpanded && (
                      <>
                        {!supabaseConn.selectedProjectId && (
                          <div className="mb-2 p-3 bg-veyra-elements-bg-depth-2 rounded-lg text-sm text-veyra-elements-textSecondary">
                        ...
                        {supabaseConn.stats?.projects?.length ? (
                          <div className="grid gap-2 max-h-60 overflow-y-auto">
                            {supabaseConn.stats.projects.map((project) => (
                              <div
                                key={project.id}
                                className="block p-3 rounded-lg border border-veyra-elements-borderColor hover:border-[#3ECF8E] dark:hover:border-[#3ECF8E] transition-colors"
                              >
                                <div className="flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-medium text-veyra-elements-textPrimary flex items-center gap-1">
                                      <div className="i-ph:database w-3 h-3 text-[#3ECF8E]" />
                                      {project.name}
                                    </h5>
                                    <div className="text-xs text-veyra-elements-textSecondary mt-1">
                                      {project.region}
                                    </div>
                                  </div>
                                  <button
                                    ...
                                    className={cn(
                                      ...
                                      supabaseConn.selectedProjectId === project.id
                                        ? 'bg-[#3ECF8E] text-white'
                                        : 'bg-veyra-elements-bg-depth-3 text-veyra-elements-textSecondary hover:bg-[#3ECF8E] hover:text-white',
                                    )}
                                  >
                                    {supabaseConn.selectedProjectId === project.id ? (
                          ...
                          <div className="text-sm text-veyra-elements-textSecondary flex items-center gap-2">
  ...
  return (
    <button
      className={cn(
        ...
        {
          'bg-veyra-elements-item-backgroundDefault hover:bg-veyra-elements-item-backgroundActive text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary':
            !active,
          'bg-veyra-elements-item-backgroundDefault text-veyra-elements-item-contentAccent': active && !disabled,
          'bg-veyra-elements-item-backgroundDefault text-alpha-gray-20 dark:text-alpha-white-20 cursor-not-allowed':
            disabled,
        },
        className,
      )}
