  ...
  ({
    ...
    useEffect(() => {
      if (data) {
        ...
        // Process veyra_event entries (blueprint errors + error_validation)
        for (const item of data) {
          if (typeof item !== 'object' || item === null || !('veyra_event' in item)) {
            continue;
          }

          const evt = (item as Record<string, unknown>).veyra_event;
    ...
    const onApiKeysChange = useCallback(
      async (providerName: string, apiKey: string) => {
        ...
        let currentCookieKeys: Record<string, string> = {};

        if (rawCookie) {
          try {
            currentCookieKeys = JSON.parse(rawCookie);
          } catch {
            currentCookieKeys = {};
          }
        }
                ...
                <>
                  {!chatStarted && (
                    <>
                      ...
                      <div className="veyra-blob veyra-blob-purple" aria-hidden="true" />
                      <div className="veyra-blob veyra-blob-blue" aria-hidden="true" />
                      <div className="veyra-blob veyra-blob-pink" aria-hidden="true" />
                      <div className="veyra-dot-grid" aria-hidden="true" />
                      ...
                      <div id="intro" className="mt-[6vh] max-w-2xl mx-auto text-center px-4 lg:px-0" style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <p className="veyra-hero-sub">
                          Describe what you want to build and Veyra will create a full-stack app, ready to ship.
                        </p>
                      </div>
                    </>
                  )}
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Example Prompts — shown below the chat box on the landing page */}
                      {!chatStarted && (
                        <div className="flex flex-wrap justify-center gap-2 mt-3 max-w-chat mx-auto w-full px-1">
                          {[
                            { icon: 'i-ph:globe', label: 'SaaS landing page' },
                            { icon: 'i-ph:chart-bar', label: 'Analytics dashboard' },
                            { icon: 'i-ph:shopping-cart', label: 'E-commerce store' },
                            { icon: 'i-ph:robot', label: 'AI chatbot UI' },
                            { icon: 'i-ph:kanban', label: 'Task manager app' },
                          ].map(({ icon, label }) => (
                            <button
                              key={label}
                              type="button"
                              onClick={() => {
                                if (handleInputChange) {
                                  const ev = { target: { value: `Build me a ${label.toLowerCase()} with a modern design` } } as React.ChangeEvent<HTMLTextAreaElement>;
                            ...
                            >
                              <span className={icon} style={{ fontSize: '14px', flexShrink: 0 }} />
                              {label}
                            </button>
                          ))}
                        </div>
...
function ScrollToBottom() {
  ...
  return (
    !isAtBottom && (
      <>
        <div className="sticky bottom-0 left-0 right-0 bg-gradient-to-t from-veyra-elements-background-depth-1 to-transparent h-20 z-10" />
        <button
          className="sticky z-50 bottom-0 left-0 right-0 text-4xl rounded-lg px-1.5 py-0.5 flex items-center justify-center mx-auto gap-2 bg-veyra-elements-background-depth-2 border border-veyra-elements-borderColor text-veyra-elements-textPrimary text-sm"
