  ...
  return (
    <>
      <div
        ...
        style={{
          background:
            'linear-gradient(180deg, var(--veyra-elements-bg-depth-3) 0%, var(--veyra-elements-bg-depth-2) 100%)',
        }}
      >
        ...
        <AnimatePresence>
          {showActions && actions.length > 0 && (
            ...
            >
              ...
              <div className="px-3 py-2 border-t border-veyra-elements-borderColor bg-veyra-elements-background-depth-2">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5">
                    <div className="i-ph:list-checks text-veyra-elements-textTertiary text-xs" />
                    <span className="text-xs text-veyra-elements-textSecondary">Actions</span>
                  </div>
                ...
                {/* Progress Bar */}
                <div className="h-0.5 bg-veyra-elements-borderColor rounded-full overflow-hidden">
                  <motion.div
              ...
              {/* Action list */}
              <div className="px-3 py-2 bg-veyra-elements-background-depth-1">
                <ActionList actions={actions} />
              </div>

              {/* Workbench button */}
              <button
                className="flex items-center gap-2 w-full px-3 py-2 border-t border-veyra-elements-borderColor hover:bg-veyra-elements-background-depth-3 transition-colors group bg-veyra-elements-background-depth-2"
                onClick={() => {
                  const showWorkbench = workbenchStore.showWorkbench.get();
                  workbenchStore.showWorkbench.set(!showWorkbench);
                }}
              >
                <div className="i-ph:code-duotone text-veyra-elements-button-primary-background text-base" />
                <div className="flex-1 text-left">
                  <div className="text-xs text-veyra-elements-textPrimary">{dynamicTitle}</div>
                  <div className="text-xs text-veyra-elements-textTertiary">Click to open Workbench</div>
                </div>
                <div className="i-ph:pencil-simple text-veyra-elements-textTertiary group-hover:text-veyra-elements-textSecondary transition-colors text-xs" />
              </button>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            >
              ...
              <button
                ...
                className={cn(
                  ...
                  'bg-veyra-elements-background-depth-2 hover:bg-veyra-elements-background-depth-3',
                  isExpanded && 'ring-1 ring-veyra-elements-borderColor',
                )}
              >
                ...
                {/* Action label */}
                <span className="text-xs text-veyra-elements-textSecondary flex-shrink-0">{actionLabel}</span>

                {/* File name */}
                {type === 'file' && fileName && (
                  <span className="text-xs font-medium text-veyra-elements-textPrimary truncate">{fileName}</span>
                )}
                ...
                {hasExpandableContent && (
                  <div
                    className={cn(
                      'transition-transform duration-200 text-veyra-elements-textTertiary',
                      !diffStats && 'ml-auto',
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <div className="mt-1.5 p-2 rounded-md bg-veyra-elements-background-depth-1 border border-veyra-elements-borderColor max-h-[200px] overflow-auto">
...
function getIconColor(status: ActionState['status']) {
  switch (status) {
    case 'pending': {
      return 'text-veyra-elements-textTertiary';
    }
    case 'running': {
      return 'text-veyra-elements-loader-progress';
    }
    case 'complete': {
      return 'text-veyra-elements-icon-success';
    }
    case 'aborted': {
      return 'text-veyra-elements-textSecondary';
    }
    case 'failed': {
      return 'text-veyra-elements-icon-error';
    }
    default: {
      return undefined;
    }
