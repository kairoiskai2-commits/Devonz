...
const ThinkingBlock = memo(({ reasoningParts }: { reasoningParts: ReasoningUIPart[] }) => {
  ...
  return (
    <div className="mb-3 rounded-lg border border-veyra-elements-borderColor overflow-hidden">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-veyra-elements-textSecondary bg-veyra-elements-background-depth-2 hover:bg-veyra-elements-background-depth-3 transition-colors"
      >
        <div className="i-ph:brain w-4 h-4 text-veyra-elements-item-contentAccent" />
        <span>Thinking</span>
        <div
      ...
      {isOpen && (
        <div className="px-3 py-2 text-xs text-veyra-elements-textSecondary bg-veyra-elements-background-depth-1 border-t border-veyra-elements-borderColor max-h-64 overflow-y-auto whitespace-pre-wrap leading-relaxed">
          {combinedText}
        </div>
      )}
    </div>
    ...
    return (
      <div className="overflow-hidden w-full">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="w-6 h-6 rounded-full bg-veyra-elements-bg-depth-3 border border-veyra-elements-borderColor flex items-center justify-center">
            <span className="text-xs font-bold text-veyra-elements-textPrimary">D</span>
          </div>
          <span className="text-sm font-medium text-veyra-elements-textSecondary">Veyra</span>
          {(codeContext || chatSummary) && (
            <Popover
              side="right"
              align="start"
              trigger={
                <div className="i-ph:info text-veyra-elements-textTertiary hover:text-veyra-elements-textSecondary transition-colors cursor-pointer" />
              }
            >
              {chatSummary && (
                <div className="max-w-chat">
                  <div className="summary max-h-96 flex flex-col">
                    <h2 className="border border-veyra-elements-borderColor rounded-md p4">Summary</h2>
                    <div style={{ zoom: 0.7 }} className="overflow-y-auto m4">
                      <Markdown>{chatSummary}</Markdown>
                    </div>
                  </div>
                  {codeContext && (
                    <div className="code-context flex flex-col p4 border border-veyra-elements-borderColor rounded-md">
                      <h2>Context</h2>
                      <div className="flex gap-4 mt-4 veyra" style={{ zoom: 0.6 }}>
                        {codeContext.map((x, i) => {
                          const normalized = normalizedFilePath(x);
                          return (
                            <Fragment key={`${normalized}-${i}`}>
                              <code
                                className="bg-veyra-elements-artifacts-inlineCode-background text-veyra-elements-artifacts-inlineCode-text px-1.5 py-1 rounded-md text-veyra-elements-item-contentAccent hover:underline cursor-pointer"
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
          ...
          {usage && (
            <div className="text-xs text-veyra-elements-textTertiary">{usage.totalTokens.toLocaleString()} tokens</div>
          )}
              ...
              {onRewind && (
                <WithTooltip tooltip="Revert to this message">
                  <button
                    ...
                    className="i-ph:arrow-u-up-left text-lg text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary transition-colors"
                  />
                </WithTooltip>
              )}
              {onFork && (
                <WithTooltip tooltip="Fork chat from this message">
                  <button
                    ...
                    key="i-ph:git-fork"
                    className="i-ph:git-fork text-lg text-veyra-elements-textTertiary hover:text-veyra-elements-textPrimary transition-colors"
                  />
                </WithTooltip>
              )}
            </div>
        ...
        {/* Message Content */}
        <div className="text-veyra-elements-textPrimary text-sm leading-relaxed">
          <Markdown
            append={append}
            chatMode={chatMode}
