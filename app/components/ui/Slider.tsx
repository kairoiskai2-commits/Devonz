...
export const Slider = genericMemo(<T,>({ selected, options, setSelected }: SliderProps<T>) => {
  ...
  return (
    <div className="flex items-center flex-wrap shrink-0 gap-1 bg-veyra-elements-background-depth-1 overflow-hidden rounded-full p-1">
...
const SliderButton = memo(({ selected, children, setSelected }: SliderButtonProps) => {
  return (
    <button
      ...
      style={{
        color: selected ? 'var(--veyra-elements-button-primary-text)' : 'var(--veyra-elements-textTertiary)',
      }}
    >
      ...
      {selected && (
        <motion.span
          ...
          style={{ background: 'var(--veyra-elements-button-primary-background)' }}
        ></motion.span>
      )}
    </button>
  );
